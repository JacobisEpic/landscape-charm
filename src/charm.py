...
"""
    ...
    def _migrate_schema(self, event: ActionEvent) -> None:
        ...
        try:
            ...
            if result.stdout:
                event.log(result.stdout)
            event.log("Schema migration completed successfully")
        except CalledProcessError as e:
            msg = f"Schema migration failed with exit code {e.returncode}."
            if e.stdout:
                msg += f" Output: {e.stdout.strip()}"
            if e.stderr:
            ...
            msg = f"Schema script not found: {SCHEMA_SCRIPT}"
            logger.error(msg)
            event.fail(msg)
            self.unit.status = BlockedStatus("Failed schema migration")
        else:
