...
class TestCharm(unittest.TestCase):
    ...
    def test_action_migrate_schema(self):
        ...
        with patch("subprocess.run") as run_mock:
            run_mock.return_value.stdout = "Migration output"
            self.harness.charm._migrate_schema(event)

        self.assertEqual(event.log.call_count, 3)
        event.fail.assert_not_called()
        run_mock.assert_called_once_with(
            [SCHEMA_SCRIPT], check=True, text=True, capture_output=True, env=ANY
        )
    ...
    def test_action_migrate_schema_CalledProcessError(self):
        ...
        with patch("subprocess.run") as run_mock:
            run_mock.side_effect = CalledProcessError(
                127, "uhoh", output="out", stderr="err"
            )
            self.harness.charm._migrate_schema(event)
    ...
    def test_action_migrate_schema_FileNotFoundError(self):
        event = Mock(spec_set=ActionEvent)
